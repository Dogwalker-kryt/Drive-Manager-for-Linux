...
// This is the TUI version of the Drive Management Utility
// TUI is like between GUI and CLI,
// v0.8.99-40-experimental --version used for the Base of TUI
// v0.0.01-01              --version of the actual TUI 
//
...
// i dont know form where it is
#include <termios.h>

// Arrow key movement and other TUI related functions
...
std::string getDriveWithTUI(const std::string& prompt, const std::vector<std::string>& drives) {
    if (drives.empty()) {
        std::cout << RED << "[ERROR] No drives found!" << RESET << "\n";
    ...
    while (true) {
        system("clear");
        std::cout << BLUE << "┌─────────────────────────────────────────────────┐" << RESET << "\n";
        std::cout << BLUE << "│" << RESET << "              " << prompt << std::string(20 - prompt.length(), ' ') << BLUE << "│" << RESET << "\n";
        std::cout << BLUE << "├─────────────────────────────────────────────────┤" << RESET << "\n";
        for (int i = 0; i < drives.size(); ++i) {
            std::cout << (i == selected ? "│ > " GREEN : "│   ") << drives[i] << std::string(35 - drives[i].length(), ' ') << BLUE << "│" << RESET << "\n";
        }
        std::cout << BLUE << "└─────────────────────────────────────────────────┘" << RESET << "\n";
        std::cout << YELLOW << "Use arrow keys to navigate, Enter to select, ESC to cancel." << RESET << "\n";
        int ch = getch();
        if (ch == 27) { // ESC
            ch = getch(); // Read [
            ch = getch(); // Read A or B
            if (ch == 65 && selected > 0) // Up
                selected--;
            else if (ch == 66 && selected < drives.size() - 1) // Down
                selected++;
            else if (ch == 67) // Right (or another key)
                return "";
        } else if (ch == 10) { // Enter
...
void printMenu(int selected) {
    system("clear");
    std::cout << BLUE << "┌─────────────────────────────────────────────────┐" << RESET << "\n";
    std::cout << BLUE << "│" << RESET << "              DRIVE MANAGEMENT UTILITY           " << BLUE << "│" << RESET << "\n";
    std::cout << BLUE << "├─────────────────────────────────────────────────┤" << RESET << "\n";
    std::cout << BLUE << (selected == 1 ? "│ > " GREEN : "│   ") << "1.  List Drives" << RESET << std::string(31, ' ') << BLUE << "│" << RESET << "\n";
    std::cout << BLUE << (selected == 2 ? "│ > " GREEN : "│   ") << "2.  Format Drive" << RESET << std::string(30, ' ') << BLUE << "│" << RESET << "\n";
    std::cout << BLUE << (selected == 3 ? "│ > " GREEN : "│   ") << "3.  Encrypt/Decrypt Drive (AES-256)" << RESET << std::string(11, ' ') << BLUE << "│" << RESET << "\n";
    std::cout << BLUE << (selected == 4 ? "│ > " GREEN : "│   ") << "4.  Resize Drive" << RESET << std::string(30, ' ') << BLUE << "│" << RESET << "\n";
    std::cout << BLUE << (selected == 5 ? "│ > " GREEN : "│   ") << "5.  Check Drive Health" << RESET << std::string(24, ' ') << BLUE << "│" << RESET << "\n";
    std::cout << BLUE << (selected == 6 ? "│ > " GREEN : "│   ") << "6.  Analyze Disk Space" << RESET << std::string(24, ' ') << BLUE << "│" << RESET << "\n";
    std::cout << BLUE << (selected == 7 ? "│ > " GREEN : "│   ") << "7.  Overwrite Drive Data" << RESET << std::string(22, ' ') << BLUE << "│" << RESET << "\n";
    std::cout << BLUE << (selected == 8 ? "│ > " GREEN : "│   ") << "8.  View Drive Metadata" << RESET << std::string(23, ' ') << BLUE << "│" << RESET << "\n";
    std::cout << BLUE << (selected == 9 ? "│ > " GREEN : "│   ") << "9.  View Info/help" << RESET << std::string(28, ' ') << BLUE << "│" << RESET << "\n";
    std::cout << BLUE << (selected == 10 ? "│ > " GREEN : "│   ") << "10. Mount/Unmount/restore (ISO/Drives/USB)" << RESET << std::string(4, ' ') << BLUE << "│" << RESET << "\n";
    std::cout << BLUE << (selected == 11 ? "│ > " GREEN : "│   ") << "11. Forensic Analysis (Beta)" << RESET << std::string(18, ' ') << BLUE << "│" << RESET << "\n";
    std::cout << BLUE << (selected == 12 ? "│ > " GREEN : "│   ") << "12. Disk Space Visualizer (Beta)" << RESET << std::string(14, ' ') << BLUE << "│" << RESET << "\n";
    std::cout << BLUE << (selected == 13 ? "│ > " GREEN : "│   ") << "13. Log viewer" << RESET << std::string(32, ' ') << BLUE << "│" << RESET << "\n";
    std::cout << BLUE << (selected == 14 ? "│ > " GREEN : "│   ") << "14. Clone a Drive" << RESET << std::string(29, ' ') << BLUE << "│" << RESET << "\n";
    std::cout << BLUE << (selected == 0 ? "│ > " GREEN : "│   ") << "0.  Exit" << RESET << std::string(38, ' ') << BLUE << "│" << RESET << "\n";
    
    std::cout << BLUE << "└─────────────────────────────────────────────────┘" << RESET << "\n";
    std::cout << YELLOW << "Use arrow keys to navigate, Enter to select." << RESET << "\n";
}
...
public:
    static void mainReader() {
        ...
        // }
            ...
            std::string driveName = getDriveWithTUI("Select a drive for metadata:", drives);
            if (driveName.empty()) return;
            try {
                DriveMetadata metadata = getMetadata(driveName);
                displayMetadata(metadata);
                std::cout << GREEN << "[SUCCESS] Metadata read successfully!" << RESET << "\n";
                std::cout << YELLOW << "Press any key to continue..." << RESET << "\n";
                getch();
            } catch (const std::exception& e) {
                std::cout << RED << "[ERROR] Failed to read metadata: " << e.what() << RESET << "\n";
                std::cout << YELLOW << "Press any key to continue..." << RESET << "\n";
                getch();
            }
    }
...
int main() {
    ...
    // return 0;
    bool running = true;
    int selected = 1; // Start with "List Drives" selected
    while (running) {
        printMenu(selected);
        int ch = getch();
        if (ch == 27) { // ESC
            ...
            if (ch == 65 && selected > 0) // Up
                selected--;
            else if (ch == 66 && selected < 14) // Down
                selected++;
        } else if (ch == 10) { // Enter
            switch (selected) {
                case 1: { // LISTDRIVES
                ...
                case 6: { // ANALYZEDISKSPACE
                    analyDiskSpace();
                    std::cout << YELLOW << "Press any key to continue..." << RESET << "\n";
                    getch();
                    break;
                }
                case 7: { // OVERWRITEDRIVEDATA
                    checkRoot();
                    std::cout << "[Warning] This function will overwrite the entire data to zeros. Proceed? (y/n)\n";
                    char zerodriveinput;
                    std::cin >> zerodriveinput;
                ...
                default: {
                    std::cout << RED << "[Error] Invalid selection" << RESET << "\n";
                    std::cout << YELLOW << "Press any key to continue..." << RESET << "\n";
                    getch();
                    break;
                }
